...
/**
 * Giao diện phục vụ: xem các bàn đang có khách (hóa đơn chưa thanh toán)
 * và thao tác CRUD món trên hóa đơn của từng bàn.
 */
public class FrmPhucVu extends JFrame {
        ...
        SwingWorker<List<BanAnModel>, Void> worker = new SwingWorker<>() {
            ...
            protected void done() {
                try {
                    ...
                    e.printStackTrace();
                    JOptionPane.showMessageDialog(FrmPhucVu.this,
    ...
    private void veLaiDanhSachBan() {
        ...
        if (danhSachBan == null || danhSachBan.isEmpty()) {
            JLabel lb = new JLabel("<html><p style='width:220px'>Chưa có bàn nào đang có khách<br>(hóa đơn chưa thanh toán).</p></html>");
    ...
    private void hienThiChiTietBan(BanAnModel ban) {
        ...
        tblModel = new DefaultTableModel(
                new String[]{"Mã món", "Tên món", "SL", "Đơn giá", "Thành tiền"}, 0) {
            @Override
    ...
    private void moHopThoaiThemMon(BanAnModel ban) {
        ...
        if (hopLe.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Không có món nào đang kinh doanh.", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        int ok = JOptionPane.showConfirmDialog(this, p, "Thêm món vào " + ban.tenBan,
        ...
        if (done) {
            ...
            JOptionPane.showMessageDialog(this, "Không thêm được món. Kiểm tra món và hóa đơn.", "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void doiSoLuongDongChon(BanAnModel ban) {
        ...
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Chọn một dòng món trong bảng.", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        String maMon = (String) tblModel.getValueAt(row, 0);
        int slCu = (Integer) tblModel.getValueAt(row, 2);
        String input = JOptionPane.showInputDialog(this,
                "Số lượng mới (0 = xóa món khỏi hóa đơn):", slCu);
        if (input == null) {
            return;
        }
        int slMoi;
