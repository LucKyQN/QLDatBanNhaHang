...
public class FrmGoiMon extends JDialog {
	...
	private JPanel createMenuArea() {
		...
		for (Entity.MonAn mon : danhSachMon) {
			...
			// Truyền thêm maMonAn vào hàm tạo Card
	...
	// --- HÀM TẠO CARD MÓN ĂN (Thêm tham số maMonAn) ---
	private JPanel createFoodCard(String emoji, String maMon, String name, double price) {
		...
		// Khi click thêm, truyền luôn maMon vào Giỏ Hàng
		btnAdd.addActionListener(e -> addToCart(maMon, name, price));
	...
	private JPanel createCartArea() {
		...
		btnGuiBep.addActionListener(e -> {
			...
			if (maHD == null) {
				JOptionPane.showMessageDialog(this, "Lỗi: Bàn này chưa được mở hóa đơn!", "Lỗi",
			...
			for (int i = 0; i < cartModel.getRowCount(); i++) {
				...
				// Gọi hàm lưu vào DB
				if (!hoaDonDAO.themHoacTangMon(maHD, maMonAn, soLuong)) {
					thanhCong = false;
				}
			}
	...
	private void addToCart(String maMon, String name, double price) {
		...
		// Duyệt bảng xem món đã có chưa
		for (int i = 0; i < cartModel.getRowCount(); i++) {
			CartItem currentItem = (CartItem) cartModel.getValueAt(i, 0);

			if (currentItem.getMaMonAn().equals(maMon)) {
		...
		// Nếu chưa có thì thêm dòng mới, BỌC TÊN VÀ MÃ VÀO CartItem
		if (!exists) {
			cartModel.addRow(new Object[] { new CartItem(maMon, name), 1, formatMoney(price) });
		}
	...
	// CLASS PHỤ TRỢ: Chứa Mã món và Tên món cho Bảng
	class CartItem {
		private String maMonAn;
		private String tenMonAn;

		public CartItem(String maMonAn, String tenMonAn) {
