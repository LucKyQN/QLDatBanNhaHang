...
//	private JLabel lbKhachHang, lbSDT, lbGioVao, lbGioRa, lbTienThua;
//	private JTextField txtKhachDua;
//	private long tongCuoiCung = 0; // Lưu tổng tiền cuối cùng để tính tiền thừa
...
//
//		// 2. KHỞI TẠO COMBOBOX TRƯỚC (Để tránh bị null khi buildBillPaper gọi tới)
...
//
//		// 3. SAU ĐÓ MỚI BUILD GIAO DIỆN
//		JPanel container = new JPanel(new BorderLayout(0, 25));
//		container.setOpaque(false);
//		container.setBorder(new EmptyBorder(30, 50, 30, 50));
//
//		container.add(buildBillPaper(ban, dsMon), BorderLayout.CENTER); // Lúc này cboKM_Current đã hết null
...
//
//		// --- PHẦN 1: HEADER (THÊM THÔNG TIN KHÁCH) ---
...
//
//		// --- PHẦN 2: DANH SÁCH MÓN (Giữ nguyên code cũ của bạn) ---
//		giaTriTamTinh = 0;
...
//		lbTienThua.setForeground(new Color(0, 150, 0)); // Màu xanh lá cho tiền thừa
//
//		// Sự kiện tính tiền thừa khi gõ số
...
//		paper.add(createSummaryRow("Khuyến mãi:", cboKM_Current, lbGiamGia)); // cboKM đã khởi tạo ở hienThiHoaDon
...
//
//		tongCuoiCung = sauGiam + phiDV + vat; // Gán vào biến toàn cục
//
//		lbGiamGia.setText("-" + formatTien(tienGiam) + " đ");
//		lbPhiDV.setText(formatTien(phiDV) + " đ");
//		lbVAT.setText(formatTien(vat) + " đ");
//		lbTongTien.setText(formatTien(Math.max(0, tongCuoiCung)) + " đ");
//
//		tinhTienThua(); // Cập nhật luôn tiền thừa khi đổi khuyến mãi
//	}
//
...
public class FrmThuNgan extends JFrame {
	...
	private void xuLyThanhToan(BanAnModel ban) {
		...
		if (khachDua < tongCuoiCung) {
			JOptionPane.showMessageDialog(this,
					"Khách đưa chưa đủ tiền để thanh toán.\n"
							+ "Khách đưa: " + formatTien(khachDua) + " đ\n"
							+ "Tổng cộng: " + formatTien(tongCuoiCung) + " đ");
			txtKhachDua.requestFocus();
			txtKhachDua.selectAll();
		...
		int opt = JOptionPane.showConfirmDialog(this,
				"Xác nhận khách đã trả tiền cho " + ban.tenBan + "?\n"
						+ "Khách đưa: " + formatTien(khachDua) + " đ\n"
						+ "Tổng cộng: " + formatTien(tongCuoiCung) + " đ\n"
						+ "Tiền thừa: " + formatTien(tienThua) + " đ",
				"Thanh toán", JOptionPane.YES_NO_OPTION);
		...
		if (dao.thanhToan(ban.maHD, 0.0, maKM)) {
			...
			int export = JOptionPane.showConfirmDialog(
					this,
					"Bạn có muốn xuất hóa đơn PDF ngay bây giờ không?",
					"Xuất hóa đơn PDF",
	...
	private void xuatHoaDonPDF(BanAnModel ban, List<MonAnModel> dsMon, long khachDua, long tienThua) {
		...
		try {
			...
			document.add(new Paragraph("Khach hang: " + (ban.tenKH != null ? ban.tenKH : "Khach vang lai"), normalFont));
			...
			document.add(new Paragraph("Gio ra: " + new SimpleDateFormat("HH:mm dd/MM/yyyy").format(new Date()), normalFont));
